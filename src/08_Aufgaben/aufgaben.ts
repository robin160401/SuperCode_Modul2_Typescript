...
// do {
//     document.write("The number is " + counter2 + "<br>");
//     counter2++;
// } while(counter2 <= 5);
// document.close();
...
//     const retrunArray: string[] = [];

//     for (let i = 1; i <= 100; i++){
//         if (i < 10) {
//             retrunArray.push("image_00" + i + ".jpg");
//         } else if (i < 100) {
//             retrunArray.push("image_0" + i + ".jpg");
//         } else {
//             retrunArray.push("image_" + i + ".jpg");
//         }
//     }
//     return (retrunArray);
// }

// const arrayOfNames: string[] = createImageNames();
// console.log(arrayOfNames);

// Aufgabe 8 + Bonusaufgabe 3

const zahlFürO = document.getElementById("zahl") as HTMLInputElement;
const loopForm = document.getElementById('loopForm') as HTMLFormElement;
let pTag = document.getElementById("loops");


function writeLoop(event: Event){
    ...
    arrayForLoop.push("p");
    let forLoop: string = arrayForLoop.join("");
    pTag!.textContent = forLoop;
}

loopForm.addEventListener("submit", writeLoop);
...
function seperateText(){
    const textInBlocksOfHunred: string[] = [];
    let   blockOfHundred: string = "";
    const gesamtZahlZeilen: number = Math.ceil(text.length / 100);
    let actualNumber: number = 0;

    for (let i = 0; i < text.length + 1; i++){
        blockOfHundred += text[i];
        if ((i +1) % 100 === 0 || i === text.length - 1){
            actualNumber++;
            blockOfHundred += " " + actualNumber + " of " + gesamtZahlZeilen;
            textInBlocksOfHunred.push(blockOfHundred);
            blockOfHundred = "";
        }
    }
    console.log(textInBlocksOfHunred);
}
...
for (let j = 0; j < numbers.length; j++){
    for (let i = 2; i < numbers[j]; i++){    
        if (numbers[j] % i === 0) {
            let result: number = numbers[j] / i ;
            console.log(numbers[j] + " is devidable by " + i  + ". The result is: " + result);
            result = 0;
        }
    }
}
